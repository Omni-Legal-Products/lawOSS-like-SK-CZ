...
- **45. Údržba nástrojovej plochy — „doctor" pre MCP, skilly a CLI** — MČ, 2026-08-18, [topic *Feature IDEAS*](https://t.me/c/3828145652/97). Podnet: v [T3 Code](https://github.com/pingdotgg/t3code) je funkcia na udržiavanie nástrojov aktuálnych; MČ používa veľa CLI (gog a spol.) a má na to vlastné upratovacie skripty — v appke by to malo zmysel **aj pre skilly**. **Konkretizuje [#23](#) (self-healing integrácie)** a dáva mu lacnejší prvý krok.
  - ⚠️ **Overené 2026-08-18 — T3 Code toto NEROBÍ.** Jeho updater pokrýva **len appku a jej server** (detekcia version skew + potvrdenie používateľom), a dokumentácia výslovne hovorí *„T3 Code drives provider CLIs; it does not ship them"* a *„does not update connected servers silently in the background"*. Aktualizáciu agentských CLI necháva na používateľa — a podľa komunitných návodov je práve zastarané CLI „takmer vždy" príčinou zlyhaní. **Čiže to nie je kopírovanie hotovej funkcie, ale nezaplnená diera v ekosystéme.**
  - **Prečo to u nás váži viac než u nich:** T3 cieli na vývojárov, ktorí si `npm i -g` spravia sami. Náš používateľ je **netechnický advokát** (zásada z `AGENTS.md` forku) — pre neho zastarané CLI alebo rozbité MCP znamená „appka nefunguje" bez cesty von.
  - **Lacný prvý krok (návrh scope):** *stavový panel* — appka vie, ktoré MCP servery, skilly a CLI potrebuje, skontroluje ich dostupnosť a verzie a ukáže stav + „je k dispozícii aktualizácia" s changelogom, jednoklikovou aktualizáciou **a rollbackom**. Plná self-healing automatika cez cron (#23) je až ďalší krok — tam treba doriešiť breaking changes.
  - 🔒 **Hranica z [ADR 0007](../decisions/0007-agent-first-architektura.md) a Q21:** aktualizácia nástroja = **stiahnutie a spustenie cudzieho kódu na stroji s klientskými dátami**. Preto: nikdy tichá inštalácia na pozadí, verzie pinnuté, aktualizáciu spúšťa výslovne človek, rollback povinný. Rovnaké pravidlo si drží aj T3 Code — a je to presne aplikácia zásady VŘ *„hranica patrí do nástroja, nie do promptu"*.
  - **Tri diely už ležia v repe:** validátor skillov + CI brána (PR #42, IR) · metodika kvality skillov (PR #35, IR) · manifest marketplace pluginov (PR #8/#9, MF — podľa [ADR 0011](../decisions/0011-proces-zmien-a-mergovania.md) sa presunie do samostatného repa). Updater je štvrtý diel, ktorý ich spája do registra skillov.
