...
- **51. TUI — terminálová alternatíva k Electron appke** *(čísla 49–50 rezervované v PR #55)* — VŘ, 2026-08-27, z quick callu. Popri desktopovej appke vyvíjať aj **Terminal User Interface** pre používateľov, ktorí preferujú terminál. Prirodzene sedí na architektúru: opencode je CLI harness a celá LAWOSS doména (OKF markdown, skills, prompty, MCP konfigy) je od začiatku prenositeľná mimo Electron (ADR 0003 — „UI default + CLI voliteľné“; ADR 0004 pravidlo 5 — portable domain layer). Prvý lacný krok už existuje: OKF spis sa dá otvoriť priamo v opencode/Claude Code bez appky. Plný TUI (register lehôt, brána v terminále) = samostatný spec, zatiaľ nezaradené do verzie.
- **52. Vlastná licencia s povinnou kontribúciou späť** — MČ, 2026-09-11, zo sync callu. Úvaha: keď niekto z konkurencie prevezme naše riešenia, mal by byť licenčne povinný prispieť späť alebo aspoň uviesť, odkiaľ to má. MF k tomu: prevzatie je prakticky isté, otázka je len, či z toho niečo máme. MČ to sám označil skôr za experiment než za vážny plán — *„nebudeme nič robiť, len to tam dáme, či sa chytia"* — a dodal, že ak to prevezmú a bude to užitočné, je to v poriadku.
  - ⚠️ **Naráža to na už prijaté rozhodnutia.** Základ je **MIT** *(vyplýva z voľby LegalWorku, [ADR 0003](../decisions/0003-legal-work-ako-zaklad.md))* a **AGPL-štýl copyleftu sme vedome zamietli** pri MikeOSS *(ADR 0002/0003)*. MIT vie vynútiť **atribúciu**, nie kontribúciu späť — povinný príspevok by znamenal zmenu licenčného režimu celého projektu, nie kozmetickú doložku.
  - **Čo sa reálne dá:** dôsledne vedený `NOTICE` a atribučná doložka *(ktorá v MIT už je)*; prípadne ochranná známka na meno a značku *(viď [ADR 0010](../decisions/0010-ochrana-know-how-a-znacky.md))* — meno chráni ochranná známka, nie licencia kódu.
  - **Ak by sa to niekedy myslelo vážne**, patrí to do ADR s porovnaním alternatív *(MIT + NOTICE × MPL-2.0 per-file copyleft × AGPL)* a s dopadom na prijatie medzi advokátmi, nie do licenčného súboru ad hoc.
